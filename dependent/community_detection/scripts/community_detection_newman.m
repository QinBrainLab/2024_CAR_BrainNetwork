function [est_network,clust_mtx] = community_detection_newman(adj_mtx, gamma)
if nargin<2
      gamma =1;
end
...
while diff_gamma>1e-5 && count<200
      [Ci, Q] = community_louvain(adj_mtx, gamma, []);
      for irep = 1:100
            [Ci_next, Q_next] = community_louvain(adj_mtx, gamma, [], 'negative_asym');
            if Q_next > Q
                  Q = Q_next; Ci = Ci_next;
            end
      end
